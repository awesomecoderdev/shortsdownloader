...
{{-- title --}}
@section('title', __('Popular Shorts Videos'))
@section('description',
    'Most Downloaded Shorts Videos by YT Short Downloader, Best free tool for Youtube shorts download online in HD
    quality. Paste the video link in the shorts downloader input box and hit the download button to process.',)
@section('url'){{ route('page.popular') }}@endsection
...
@section('content')
    ...
    <div class="shorts">
        <div class="max-w-screen-xl mx-auto p-5 sm:p-6 md:p-8 lg:p-4 xl:p-5">
            <div id="shorts" data-token="{{ csrf_token() }}" data-load="{{ route('loadmore') }}" data-option="popular"
                data-page="1"
    ...
    <div id="loadmore" class="container flex justify-center relative py-4" @click.prevent="loadMore($event)"
        data-token="{{ csrf_token() }}" data-load="{{ route('loadmore') }}" data-option="popular">
        <button
